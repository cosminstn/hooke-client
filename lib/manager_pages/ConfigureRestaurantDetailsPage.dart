...
Future<Restaurant> fetchRestaurantByCreator() async {
  ...
  final response = await client.get(
      Constants.API_BASE_URL + '/pub/restaurants/bycreator',
      headers: {
        'APP_TOKEN': Constants.APP_TOKEN,
        'Authorization' : Globals.authToken
      }
    );
  return compute<String, Restaurant>(parseSingleRestaurant, response.body);
}

Restaurant parseSingleRestaurant(String responseBody) {
  final parsed = json.decode(responseBody).cast<Map<String, dynamic>>();
  return parsed.map<Restaurant>((json) => Restaurant.fromJson(json));
}
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<Restaurant> (
      ...
      builder: (context, snapshot) {
        ...
        return snapshot.hasData ? _RestaurantDetailsForm(dbRestaurantInstance: snapshot.data)
